...
<div class="row">
  <div class="col-12">
    <form method="POST" action="{{ route('aboutcompany.update', $aboutcompany->id) }}" enctype="multipart/form-data">
      {{ method_field('PUT') }}
      @csrf
        ...
        <div class="card-body">
          ...
          <div class="form-group col-md-12">
            ...
            <div style="display: flex; flex-direction: row">
              <input type="text" name="title" class="form-control col-sm-12" required value="{{ old('title', $aboutcompany->title) }}" placeholder="{{__('Title')}}" />
            </div>
          </div>
          <div class="form-group col-md-12">
...
<script>
  tinymce.init({
    ...
    setup: function(editor) {
      editor.on('init', function(e) {
        editor.setContent("{{ $aboutcompany->embed }}");
      });
    }
  });
  ...
  //   }
  // }, schema);
  // parser.parse("{{$aboutcompany->embed}}");
  // tinymce.activeEditor.setContent("{{$aboutcompany->embed}}");
</script>
<!-- tinymce editor -->
...
<script>
  ...
  $("#btnDeleteClient").click(function() {
    if (confirm("{{__('Would you delete this Client ?')}}") == false)
      return false;
    $.ajax({
      url: "{{ route('aboutcompany.destroy', $aboutcompany->id) }}",
      headers: {

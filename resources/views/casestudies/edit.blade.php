...
<div class="row">
  <div class="col-12">
    <form method="POST" action="{{ route('casestudies.update', $casestudy->id) }}" enctype="multipart/form-data">
      {{ method_field('PUT') }}
      @csrf
        ...
        <div class="card-body">
          ...
          <div class="form-group col-md-12">
            ...
            <div style="display: flex; flex-direction: row">
              <input type="text" name="title" class="form-control col-sm-12" required value="{{ old('title', $casestudy->title) }}" placeholder="{{__('Title')}}" />
            </div>
          </div>
          <div class="form-group col-md-12">
...
<script>
  tinymce.init({
    ...
    setup: function(editor) {
      editor.on('init', function(e) {
        editor.setContent("{{ $casestudy->embed }}");
      });
    }
  });
  ...
  //   }
  // }, schema);
  // parser.parse("{{$casestudy->embed}}");
  // tinymce.activeEditor.setContent("{{$casestudy->embed}}");
</script>
<!-- tinymce editor -->
...
<script>
  ...
  $("#btnDeleteClient").click(function() {
    if (confirm("{{__('Would you delete this Client ?')}}") == false)
      return false;
    $.ajax({
      url: "{{ route('casestudies.destroy', $casestudy->id) }}",
      headers: {
      ...
      data: {
        "_token": "{{ csrf_token() }}",
        "id": "{{$casestudy->id}}" // method and token not needed in data
      },

...
<h1 class="m-0 text-dark">{{__('Add News And Events')}}</h1>
@stop
...
<div class="row">
  <div class="col-12">
    ...
    @endif
    <form method="POST" action="{{ route('news_events.store') }}" id="frmCreateClient" enctype="multipart/form-data">
      @csrf
      <input type="hidden" id="isautoplaceorder" name="isautoplaceorder" value="0" />
      <div class="card card-info">
        <div class="card-header">
          <h3 class="card-title">{{__('Add News And Events')}} </h3>
        ...
        <div class="card-body">
          ...
          <div class="form-group col-md-12" id="articleid" style="display: none;">
            ...
            <select class="form-control col-sm-12" name="articleid" id="compareSelector" v-model="locale">
              ...
              <option value="{{ $item->articleid }}" label="{{ \Soundasleep\Html2Text::convert($item->title, ['ignore_errors' => true, 'drop_links' => true]) }}"></option>
              @endforeach
            </select>
          </div> -->
          <div class="form-group col-md-12">

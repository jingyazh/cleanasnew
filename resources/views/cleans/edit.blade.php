...
<div class="row">
  <div class="col-12">
    <form method="POST" action="{{ route('cleans.update', $clean->id) }}" enctype="multipart/form-data">
      {{ method_field('PUT') }}
      @csrf
        ...
        <div class="card-body">
          ...
          <div class="form-group col-md-12">
            ...
            <div style="display: flex; flex-direction: row">
              <input type="text" name="title" class="form-control col-sm-12" required value="{{ old('title', $clean->title) }}" placeholder="{{__('Title')}}" />
            </div>
          </div>
          <div class="form-group col-sm-12">
            ...
            <select class="form-control col-sm-12" name="type" v-model="type">
              ...
              <option value="Heat Exchangers" label="Heat Exchangers" {{ $clean->type == 'Heat Exchangers' ? "selected" : ''}}></option>
              <option value="Parts" label="Parts" {{ $clean->type == 'Parts' ? "selected" : ''}}></option>
            </select>
          </div>
          <div class="form-group col-md-12">
            ...
            <small id="passwordHelpBlock" class="ul-form__text form-text ">
              View example <a href="/assets/examples/whatweclean.jpg" target="_blank">here</a> | Current Image <a href="/{{ $clean->image }}" target="_blank">here</a>
            </small>
          </div>
...
<script>
  ...
  $("#btnDeleteClient").click(function() {
    ...
    $.ajax({
      ...
      data: {
        "_token": "{{ csrf_token() }}",
        "id": "{{$clean->id}}" // method and token not needed in data
      },

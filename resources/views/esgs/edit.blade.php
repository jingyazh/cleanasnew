...
<div class="row">
  <div class="col-12">
    <form method="POST" action="{{ route('esgs.update', $esg->id) }}" enctype="multipart/form-data">
      {{ method_field('PUT') }}
      @csrf
        ...
        <div class="card-body">
          ...
          <div class="form-group col-md-12">
            ...
            <div style="display: flex; flex-direction: row">
              <input type="text" name="title" class="form-control col-sm-12" required value="{{ old('title', $esg->title) }}" placeholder="{{__('Title')}}" />
            </div>
          </div>
          <div class="form-group col-md-12">
...
<script>
  function cancel() {
    location.href = "{{ route('esgs.index') }}";
    return false;
  }
</script>
<!-- tinymce editor -->
...
<script>
  ...
  //   }
  // }, schema);
  // parser.parse("{{$esg->embed}}");
  // tinymce.activeEditor.setContent("{{$esg->embed}}");
</script>
<!-- tinymce editor -->
...
<script>
  ...
  $("#btnDeleteClient").click(function() {
    ...
    $.ajax({
      ...
      data: {
        "_token": "{{ csrf_token() }}",
        "id": "{{$esg->id}}" // method and token not needed in data
      },

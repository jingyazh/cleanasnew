...
<h1 class="m-0 text-dark">{{__('Add ESG')}}</h1>
@stop
...
<div class="row">
  <div class="col-12">
    ...
    @endif
    <form method="POST" action="{{ route('esgs.store') }}" id="frmCreateClient" enctype="multipart/form-data">
      @csrf
      <input type="hidden" id="isautoplaceorder" name="isautoplaceorder" value="0" />
      <div class="card card-info">
        <div class="card-header">
          <h3 class="card-title">{{__('Add ESG')}} </h3>
        ...
        <div class="card-body">
          ...
          @endif
          <div class="form-group col-md-12" id="esgid" style="display: none;">
            <label>{{__('Select title')}}<code>*</code> </label>
            <select class="form-control col-sm-12" name="esgid" id="compareSelector" v-model="locale">
              <option value="" label="Please select title" selected></option>
              @foreach ($esgs as $key => $item)
              <option value="{{ $item->esgid }}" label="{{ $item->title }}"></option>
              @endforeach
            </select>
          </div> -->
          <div class="form-group col-md-12">

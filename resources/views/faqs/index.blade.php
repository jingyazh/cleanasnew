        ...
        <div class="card-body">
          <div class="form-group col-md-12">
            ...
            <div style="display: flex; flex-direction: row">
              <input type="text" name="faq_meta_title" class="form-control col-sm-12" value="{{isset($setting) ? $setting->faq_meta_title : ''}}" required placeholder="{{__('Meta Title')}}" />
            </div>
          </div>
          ...
          <div class="form-group col-md-12">
            ...
            <div style="display: flex; flex-direction: row">
              <input type="text" name="faq_meta_description" class="form-control col-sm-12" value="{{isset($setting) ? $setting->faq_meta_description : ''}}" required placeholder="{{__('Meta Description')}}" />
            </div>
          </div>
        </div>
        <!-- /.card-body -->
...
<script>
  ...
  $(function() {
    ...
    oTable = $("#listtable").DataTable({
      processing: true,
      serverSide: true,
      ajax: "{{ route('faqs.data', ['locale' => str_replace('_', '-', app()->getLocale())]) }}",
